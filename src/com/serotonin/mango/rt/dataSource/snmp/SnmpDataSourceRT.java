...
public class SnmpDataSourceRT extends PollingDataSource {
    ...
    public SnmpDataSourceRT(SnmpDataSourceVO vo) {
        ...
        version = Version.getVersion(vo.getSnmpVersion(), vo.getCommunity(), vo.getSecurityName(),
                vo.getAuthProtocol(), vo.getAuthPassphrase(), vo.getPrivProtocol(), vo.getPrivPassphrase(),
                vo.getEngineId(), vo.getContextEngineId(), vo.getContextName());
    }
    ...
    public void setPointValue(DataPointRT dataPoint, PointValueTime valueTime, SetPointSource source) {
        ...
        PDU response = snmp.set(request, target).getResponse();
    ...
    protected void doPoll(long time) {
        try {
            ...
            raiseEvent(PDU_EXCEPTION_EVENT, time, true, DataSourceRT.getExceptionMessage(e));
        }
    }
    ...
    private void doPollImpl(long time) throws IOException {
        ...
        // Make a copy of the oids so that we can check if we got everything we asked for, and
            ...
            for (int i = 0; i < response.size(); i++) {
                ...
                if (dp != null) {
                    ...
                    if (vb.getVariable().isException()) {
                        error = true;
                        raiseEvent(PDU_EXCEPTION_EVENT, time, true, new LocalizableMessage("event.snmp.oidError",
                                address, getOid(dp), variable.toString()));
                    }
                    else
                    ...
                    raiseEvent(PDU_EXCEPTION_EVENT, time, true,
                            new LocalizableMessage("event.snmp.unknownOid", vb.getOid(), address));
                }
            }

            for (DataPointRT requestPoint : requestPoints) {
                error = true;
                raiseEvent(PDU_EXCEPTION_EVENT, time, true, new LocalizableMessage("event.snmp.noBinding",
    ...
    private LocalizableMessage validatePdu(PDU pdu) {
        ...
        if (pdu.getErrorIndex() != 0)
            return new LocalizableMessage("event.snmp.pduOidError", pdu.get(pdu.getErrorIndex() - 1).getOid(),
                    pdu.getErrorStatusText());

        if (pdu.getErrorStatus() != 0)
            return new LocalizableMessage("event.snmp.pduErrorStatus", pdu.getErrorStatus(), pdu.getErrorStatusText());
